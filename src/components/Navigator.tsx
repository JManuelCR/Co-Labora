...
export default function Navbar(props: Navbar) {
    ...
    return(
        ...
        <nav className="flex items-center justify-between w-full max-h-[56px] max-w-[1440px]">
          <div>
            <Link href="/">
              <Image
                src={logo}
          ...
          <div className="flex gap-[35px] items-center">
            <ul className=" flex gap-[35px] text-[14px] text-blue_700 font-semibold">
              <li><Link href={"/"} className={isActive === 'home' ? 'navItemFocused' : ''} onClick={handleNavHomeClick}>Home</Link></li>
              <li><Link href={"/your-spaces"} className={isActive === 'your spaces' ? 'navItemFocused' : ''} onClick={handleNavYourSpacesClick}>Tus espacios</Link></li>
              <li><Link href={"/in-rent"} className={isActive === 'in rent' ? 'navItemFocused' : ''} onClick={handleNavInRentClick}>En renta</Link></li>
              <li> <Link href={"/contacts"} className={isActive === 'contacts' ? 'navItemFocused' : ''} onClick={handleNavContactsClick}>Contactos</Link></li>
            </ul>
